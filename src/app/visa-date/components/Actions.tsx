...
import { ISlot, ISlotsSuccessResponse } from "@/utils/solts";

interface IProps {
  data: RequestPayload;
}
const Actions: React.FC<IProps> = ({ data }) => {
  ...
  const [timesSlotsResponse, setTimesSlotsResponse] =
    useState<ISlotsSuccessResponse | null>(null);
  ...
  // const slotDatesAndTimes = () => {
  //   const datesAndTimes = verifyResponse?.data?.slot_dates?.map((date) => {
  //     console.log(date);
  //     return {
  //       date: date,
  //       times: "",
  ...
  const handleGetTimeSlots = async () => {
    ...
    data?.info?.forEach((item) => {
      item.appointment_time = selectedSlotDate;
      item.otp = otp;
    });
  ...
  const handlePayment = async () => {
    ...
    data.otp = otp;
    data.selected_slot = timesSlotsResponse?.slot_times[selectedTimeSlotIndex];
    delete data.resend;
    await payInvoice(
      setIsLoading,
      setPaymentResponse,
    ...
    <Grid container spacing={2}>
            ...
            otpResponse ? (
              <Box>
                <Box width="100%">
                  ...
                  >
                    OTP sent successfully, please check your inbox. and verify
                    first.
                  </Typography>
                </Box>
          ...
          {selectedSlotDate && (
            <Box paddingY="10px">
              ...
              {timesSlotsResponse ? (
                ...
                >
                  {timesSlotsResponse?.slot_times?.map((slot, i) => (
                    <Paper
                    ...
                    >
                      ...
                      >
                        {`${slot?.time_display} | Available Slots - ${slot?.availableSlot}`}
                      </Typography>
                  ...
                  {/* <code>
                    <pre>{JSON.stringify(timesSlotsResponse, null, 2)}</pre>
                  </code> */}
                </Box>
              ) : (
                <Button
          ...
          {timesSlotsResponse && (
            <Box sx={{ width: "100%", marginY: "10px" }}>
              <Grid container spacing={1}>
                {Object.keys(paymentOptions)?.map((key: string, i: number) => (
                  <Grid item xs={4} key={i}>
                    ...
                    >
                      <img
                        ...
                        src={
                          paymentOptions[key as keyof typeof paymentOptions]
                            ?.link
                        }
                        alt="bkash"
                        width="100%"
              ...
              {paymentResponse && (
                <Box marginY="10px">
                  <Typography>{JSON.stringify(paymentResponse)}</Typography>
                </Box>
              )}
            </Box>
          )}
      ...
      <Grid item xs={6}>
        <DirectActions
          ...
          selectedSlotDate={selectedSlotDate as string}
          setIsLoading={setIsLoading}
          setPaymentResponse={setPaymentResponse}
          setErrorMessages={setErrorMessages}
          responseData={paymentResponse as any}
        />
      </Grid>
    </Grid>
  );
